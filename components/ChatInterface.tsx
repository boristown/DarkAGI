...
import { Send, Upload, Loader2, Sparkles, Terminal, ChevronDown, ChevronRight, Check, Menu, X, File as FileIcon, Trash2, Lightbulb, Eye, Zap, Globe, Square, Code, Image as ImageIcon, Film, Copy } from 'lucide-react';
...
// Sub-component to render individual user attachments with preview
const UserAttachment: React.FC<{ file: VirtualFile }> = ({ file }) => {
  const [objectUrl, setObjectUrl] = useState<string | null>(null);
  
  // Determine if it is an image based on mimeType or extension
  ...
  // Fallback for non-image files (Chip style)
  return (
...
const InlineFilePreview: React.FC<{ path: string, files: VirtualFile[] }> = ({ path, files }) => {
  ...
  return (
    <div className="mt-3 rounded-lg overflow-hidden border border-gray-800 bg-black/20 inline-block max-w-full">
        {fileType === 'image' && (
            <img src={objectUrl} alt={path} className="max-h-64 object-contain" />
        )}
        {fileType === 'video' && (
            <video src={objectUrl} controls className="max-h-64 max-w-full" />
        )}
    ...
    <div className="flex items-start gap-3 p-3 rounded-lg bg-gray-900/50 border border-gray-800/50 mb-2 font-mono text-xs">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1 flex-wrap">
          <span className={`px-1.5 py-0.5 rounded border uppercase text-[10px] font-bold ${getBadgeColor(action.type)}`}>
            {action.type}
          </span>
        ...
        {/* Inline Preview for Media Actions */}
        {status === 'completed' && isMediaAction && (
            <InlineFilePreview path={action.path} files={files} />
        )}
      </div>
    </div>
  );
...
// Isolated Code Renderer Component to manage copy state
const CodeRenderer = ({ node, inline, className, children, ...props }: any) => {
    ...
    if (inline) {
        return (
            <code className="bg-gray-800/80 text-purple-300 px-1.5 py-0.5 rounded text-[13px] font-mono border border-gray-700/50" {...props}>
                {children}
            </code>
        );
    }
    ...
    return (
        <div className="relative group my-4 rounded-lg overflow-hidden border border-gray-800 bg-[#1e1e1e] shadow-lg">
            <div className="flex items-center justify-between px-3 py-1.5 bg-[#252526] border-b border-gray-700/30">
                ...
                <button
                    onClick={handleCopy}
                    className="flex items-center gap-1.5 text-[10px] text-gray-400 hover:text-white transition-colors p-1 rounded hover:bg-gray-700/50 active:scale-95"
...
const AgentResponseBubble = ({ message, onExecute, t, isLatest, files }: { message: ChatMessage, onExecute: (actions: AgentAction[]) => void, t: any, isLatest: boolean, files: VirtualFile[] }) => {
  // Auto-expand thought if it looks long or if it's the latest message (streaming)
  const isComplexThought = message.agentResponse?.thought && message.agentResponse.thought.length > 500;
  ...
  const hasRaw = !!agentResponse.raw && isLatest; // Only show raw option when streaming/latest for debugging
    ...
    <div className="flex flex-col gap-4 w-full max-w-3xl animate-fade-in">
      ...
      <div className="bg-gray-800/30 border border-gray-700/50 rounded-xl overflow-hidden">
        ...
        >
          ...
          <span className="text-xs text-gray-500 mr-2 truncate max-w-[200px]">
             {agentResponse.thought.slice(0, 50)}...
          </span>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 shadow-xl relative overflow-hidden">
        <div className="absolute top-0 right-0 p-4 opacity-10">
             {hasActions ? <Terminal className="w-24 h-24" /> : <Sparkles className="w-24 h-24" />}
        </div>
            ...
            <div className="mb-6 relative z-10">
                ...
                <div className="text-gray-200 text-sm leading-7 prose prose-invert prose-sm max-w-none">
                    <ReactMarkdown components={markdownComponents}>
                        {(agentResponse.final_answer || '').length > 10000 
                            ? (agentResponse.final_answer || '').slice(0, 10000) + '\n\n... [Content truncated (>10000 chars)]' 
                ...
                <h3 className="text-sm font-semibold text-blue-400 mb-3 flex items-center gap-2">
                 ...
                 <ul className="space-y-2">
                    {agentResponse.plan.map((step, idx) => (
                      <li key={idx} className="flex items-start gap-2 text-sm text-gray-400 animate-fade-in">
        ...
        {hasActions && (
          <>
             ...
             <h3 className="text-sm font-semibold text-blue-400 mb-4 flex items-center gap-2 relative z-10">
               <Terminal className="w-4 h-4" />
               {t.executedActions}
             </h3>
            ...
            <div className="border-t border-gray-800 pt-4 mb-4 relative z-10">
               {agentResponse.actions.map((action, idx) => (
                 <ActionCard 
                    key={action.id} 
                    action={action} 
                    status={'completed'}
      ...
      {/* Raw Output Log (Debug View) */}
      {hasRaw && (
        <div className="bg-black/30 border border-gray-800 rounded-lg overflow-hidden mt-2">
             <button 
                onClick={() => setIsRawExpanded(!isRawExpanded)}
                className="w-full flex items-center gap-2 px-3 py-2 text-xs text-gray-500 hover:text-gray-300 hover:bg-gray-800/50 transition-colors"
             >
                <Code className="w-3 h-3" />
                <span>{t.rawOutput}</span>
                <div className="flex-1" />
                {isRawExpanded ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
             </button>
             {isRawExpanded && (
                <div className="p-3 bg-black/50 border-t border-gray-800 font-mono text-[10px] text-gray-400 whitespace-pre-wrap break-all max-h-60 overflow-y-auto">
                    {agentResponse.raw}
                </div>
             )}
        </div>
      ...
      <div className="h-16 border-b border-gray-800 flex items-center justify-between px-4 bg-gray-900/95 backdrop-blur-sm sticky top-0 z-20 shadow-md">
        <div className="flex items-center gap-3">
            ...
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-gray-900 via-indigo-900 to-purple-900 flex items-center justify-center text-white shadow-[0_0_15px_rgba(99,102,241,0.25)] border border-indigo-500/30 relative overflow-hidden group transition-all hover:shadow-[0_0_25px_rgba(139,92,246,0.5)] hover:border-indigo-400/50">
                ...
                {/* Custom D-Bolt Icon */}
                <svg viewBox="0 0 24 24" className="w-6 h-6 text-white drop-shadow-md transform group-hover:scale-110 transition-transform duration-300" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                   {/* The Bolt forming the left spine of the D */}
                   <path d="M10 2L4 13h6l-2 9" className="text-indigo-400" strokeWidth="3" />
                   {/* The Curve forming the right side of the D */}
                   <path d="M10 2c5 0 9 4.5 9 10s-4 10-9 10" className="opacity-90" />
                </svg>
            </div>
            
            <h1 className="font-display font-bold text-xl text-transparent bg-clip-text bg-gradient-to-r from-gray-100 to-gray-400 hidden sm:block tracking-wide">
              {t.appName} <span className="text-[10px] bg-gray-800 text-gray-400 px-1.5 py-0.5 rounded-sm border border-gray-700 font-mono align-middle ml-1 relative -top-0.5">{t.version}</span>
            </h1>
        </div>
        <div className="flex items-center gap-3">
             {/* Language Toggle */}
            <button
                ...
                className="flex items-center gap-1.5 px-2 py-1.5 text-xs font-medium text-gray-400 hover:text-white hover:bg-gray-800 rounded-lg transition-all border border-transparent hover:border-gray-700"
            ...
            <div className="relative group">
                <select 
                    ...
                    className="appearance-none bg-gray-800 border border-gray-700 text-gray-300 text-xs font-medium rounded-lg pl-3 pr-8 py-2 outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500/50 transition-all hover:bg-gray-750 cursor-pointer min-w-[140px]"
                >
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 md:space-y-8 scroll-smooth">
        {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-gray-600 space-y-6 px-4 animate-fade-in-up">
                <div className="relative group">
                  <div className="absolute inset-0 bg-purple-500/30 blur-3xl rounded-full opacity-50 group-hover:opacity-70 transition-opacity duration-1000"></div>
                  <div className="bg-gray-900/50 p-6 rounded-2xl border border-gray-800/50 relative backdrop-blur-sm">
                      {/* Big Initial State Icon matching the D-Bolt theme */}
                      <svg viewBox="0 0 24 24" className="w-12 h-12 text-purple-400 relative z-10 drop-shadow-[0_0_15px_rgba(168,85,247,0.5)]" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                         <path d="M10 2L4 13h6l-2 9" />
                         <path d="M10 2c5 0 9 4.5 9 10s-4 10-9 10" />
                      </svg>
                  </div>
                </div>
                <div className="text-center space-y-3">
                  <h2 className="font-display text-3xl font-bold text-gray-200 tracking-tight">
                    {t.initializeTitle}
                  </h2>
                  <p className="text-sm text-gray-500 max-w-sm mx-auto font-mono leading-relaxed">
                    {t.initializeDesc}
                  </p>
                </div>
            </div>
        ...
        {messages.map((msg, index) => (
          <div key={msg.id} className={`flex ${msg.role === 'user' && !msg.isObservation ? 'justify-end' : 'justify-start'}`}>
            {/* User Message */}
            {msg.role === 'user' && !msg.isObservation && (
              <div className="bg-gradient-to-br from-gray-800 to-gray-800/80 text-gray-100 rounded-2xl rounded-tr-sm px-5 py-3.5 max-w-[90%] md:max-w-xl text-sm leading-relaxed shadow-lg border border-gray-700/50 group">
                <div className="whitespace-pre-wrap">{msg.content}</div>
                {msg.attachments && msg.attachments.length > 0 && (
                    <div className="mt-3 pt-3 border-t border-gray-700/50 flex flex-wrap gap-2">
                        {msg.attachments.map((f, i) => (
                            <UserAttachment key={i} file={f} />
                        ))}
                    </div>
                )}
              </div>
            ...
            {/* System/Observation Message */}
            {msg.role === 'user' && msg.isObservation && (
                <div className="flex gap-4 max-w-3xl w-full pl-2">
                    <div className="w-8 h-8 rounded-lg bg-gray-900 flex items-center justify-center flex-shrink-0 border border-gray-800 relative">
                        <div className="absolute inset-0 bg-blue-500/5 rounded-lg"></div>
                        <Eye className="w-4 h-4 text-gray-500" />
                    </div>
                    <div className="flex-1 bg-gray-950/30 border border-gray-800/50 rounded-lg p-3 font-mono text-[11px] text-gray-400 whitespace-pre-wrap leading-relaxed shadow-inner">
                        <span className="text-blue-500/80 font-bold mr-2">{t.systemObservation}:</span>
                        {msg.content.length > 10000 
                            ? msg.content.slice(0, 10000) + '... [Truncated]' 
                            : msg.content}
                    </div>
                </div>
            )}
              ...
              <div className="w-full max-w-3xl">
                {msg.agentResponse ? (
                    <AgentResponseBubble 
                        ...
                        onExecute={() => {}} 
                        t={t} 
                        isLatest={index === messages.length - 1 && isProcessing}
                        files={files}
                    />
                ) : (
                    <div className={`text-xs md:text-sm p-4 border rounded-lg backdrop-blur-sm ${
                        msg.isError && msg.content.includes(t.stopped) 
                        ? 'text-yellow-400 bg-yellow-900/10 border-yellow-900/20'
                        : 'text-red-400 bg-red-900/10 border-red-900/20'
                    }`}>
                        <span className="font-bold">{msg.isError && msg.content.includes(t.stopped) ? 'System' : t.systemError}:</span> {msg.content}
                    </div>
                )}
              </div>
            )}
        ...
        {isProcessing && (
          <div className="flex justify-start animate-fade-in pl-2">
            <div className="bg-gray-900/60 rounded-full px-5 py-3 flex items-center gap-3 border border-purple-500/20 shadow-[0_0_15px_rgba(168,85,247,0.1)]">
              <Loader2 className="w-4 h-4 animate-spin text-purple-500" />
              <span className="text-xs font-mono text-purple-300/80 tracking-wide">{t.processing}</span>
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="max-w-4xl mx-auto flex flex-col gap-2 bg-gray-800/80 p-2.5 rounded-2xl border border-gray-700/50 shadow-2xl focus-within:border-purple-500/50 focus-within:ring-1 focus-within:ring-purple-500/20 transition-all duration-300">
          ...
          {pendingFiles.length > 0 && (
            <div className="flex gap-2 overflow-x-auto pb-2 px-1 scrollbar-hide">
              {pendingFiles.map((file, idx) => (
                <div key={idx} className="flex-shrink-0 flex items-center gap-2 bg-gray-700/50 pl-3 pr-2 py-1.5 rounded-lg text-xs text-gray-300 border border-gray-600/30">
                  <span className="truncate max-w-[120px]">{file.name}</span>
                  <button 
                    type="button"
                    onClick={() => removePendingFile(idx)}
                    className="p-0.5 hover:bg-gray-600 rounded-full text-gray-400 hover:text-white transition-colors"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </div>
          ...
          <div className="flex items-end gap-2.5">
            ...
            <button 
              ...
              className="p-3 text-gray-400 hover:text-purple-400 hover:bg-gray-700/50 rounded-xl transition-all active:scale-95 group"
              title={t.uploadHint}
            ...
            <textarea
              ...
              className="flex-1 bg-transparent border-none outline-none text-gray-200 text-sm min-h-[44px] max-h-32 py-3 px-1 resize-none placeholder-gray-500/70 leading-relaxed font-medium"
              rows={1}
            />
            ...
            <button 
              ...
              className={`p-3 rounded-xl transition-all shadow-lg active:scale-95 flex items-center justify-center ${
                  isProcessing 
                    ? 'bg-red-500/10 hover:bg-red-500/20 text-red-500 border border-red-500/50 shadow-red-900/20' 
                    : 'bg-gradient-to-br from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white shadow-purple-900/20 disabled:opacity-50 disabled:grayscale disabled:shadow-none'
              }`}
              title={isProcessing ? t.stop : undefined}
            >
